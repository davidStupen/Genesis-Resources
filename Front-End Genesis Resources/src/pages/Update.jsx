...
const Update = () => {
  ...
  const handlerFullNameSubmit = async (e) => {
    e.preventDefault() 
    try{
      const response = await axios.put("http://13.60.230.45:8080/api/v1/users", fullName)
      navigate("/")
    } catch(err){
      console.error(err)
    }
  ...
  useEffect(() => {
    ...
    try {
      const fetch = async () => {
        const response = await axios.get(`http://13.60.230.45:8080/api/v1/users/${currentId}`)
